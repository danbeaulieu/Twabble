...
TWITTERAUTH_KEY = getattr(settings, 'CONSUMER_KEY', 'OH HAI')
TWITTERAUTH_SECRET = getattr(settings, 'CONSUMER_SECRET', 'OH NOES')
...
def get_authorized_token(token, signature_method=signature_method):
	...
	req.sign_request(signature_method, consumer(), token)
	resp_st = oauth_response(req)
	return oauth.OAuthToken.from_string(resp_st)
